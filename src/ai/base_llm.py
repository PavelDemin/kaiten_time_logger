...
class BaseLLM(ABC):
    ...
    def create_prompt(commits: List[str], language: str = 'ru') -> str:
        ...
        if language == 'ru':
            return (
                f'Проанализируй коммиты по задаче и создай краткое описание проделанной работы '
                f'(не более 2-3 предложений). В коммитах могут быть временные метки, идентификаторы задачи и другая '
                f'косвенная информация, игнорируй эти метки при составлении описания. Не переводи на русский язык '
                f'названия переменных, методов, классов и других атрибутов используемых в программном коде: '
